...
def all_vowel_pairs(words)
    ...
    (0...words.length).each do |idx1|
        hash_1 = vowel_count(words[idx1])
        (idx1+1...words.length).each do |idx2|
            hash_2 = vowel_count(words[idx2])
            total = hash_1.merge(hash_2)
            check = total.keys.sort.join
            if check == "aeiou"
                pairs.push([words[idx1], words[idx2]].join(" "))
            end
            hash_2 = {}
        end
        hash_1 = {}
...
def find_bigrams(str, bigrams)
    ...
    #         new_arr << str[idx1..idx2] if bigrams.include?(str[idx1..idx2])
    #     end
    # end
    # new_arr.sort
...
class String
    ...
    def caesar_cipher(num)
        ...
        self.each_char do |ele|
            new_str += alphabet[(alphabet.index(ele) + num) % 26]
        end
        new_str
    end
end
